...
var print = print;
var require = require;
var _setTimeout = require("ringo/scheduler").setTimeout;
 ...
 * @param {Boolean} [useProdAPNS] - defaults to the value set in application.ffdl for "ApplePushUsingProduction"
 * WARNING: Synchronous sending is for TESTING purposes only; you should never use synchronous sending in production
 */
function sendIOSPush (notificationToken, payload, certFileName, certPassword, sendSynchronously, useProdAPNS) {
    ...
    if (useProdAPNS === true || useProdAPNS === false) { // Specific over-ride used - call appropriate method signature
        return FF_JS_API.sendIOSPush (notificationToken, payload, certFileName, certPassword, !!sendSynchronously, useProdAPNS);
    } else {
 ...
 * <br><strong>Note: </strong> This will return extremely quickly when executed as the system user. However when executed as a different user,
 * <br>the security access controls are exercised - this requires that all objects are retrieved from the database and then filtered via security
 * <br>For queries which return very large numbers of objects, this is necessarily a couple of orders of magnitude slower.
 * @param {String} query
 * @param {String} [asUserGuid] Defaults to 'system'. The operation is executed as if it were this user, so all security access controls will be in effect.
 * @return {Number}
 * @example
...
/**
 * Turns XML into JSON. (NB: This is currently a BETA feature)
 * @param {String} xml
 * @returns {String}
 */
 ...
 * @param {String} mutexName - for example, the ffUrl of an object you want to lock for some reason
 * @param {Number} timeoutInSeconds - mutex will be auto-unlocked after this many seconds (in the event that for some reason unlockMutex isn't called by your code)
 * @returns {Boolean} true if locked, false otherwise
 */
function lockMutex (mutexName, timeoutInSeconds) {
    if (typeof mutexName !== 'string') {
        throw new Error("ERROR: mutexName (string) must be supplied")
    }
    if (parseInt(timeoutInSeconds) < 1) {
        throw new Error("ERROR: timeoutInSeconds (positive integer) must be supplied")
    }
    return FF_JS_API.lockMutex(mutexName, timeoutInSeconds);
}
 ...
 * @param {String} mutexName - for example, the ffUrl of an object you want to lock for some reason
 */
function unlockMutex (mutexName) {
    if (typeof mutexName !== 'string') {
        throw new Error("ERROR: mutexName (string) must be supplied")
    }
    return FF_JS_API.unlockMutex(mutexName);
}
exports.unlockMutex = unlockMutex;

/**
 * @param {String} base64PublicKey the base64-encoded public key to use for verifying.
 * @param {String} signedData the signed JSON string - signed, not encrypted
 * @param {String} signature the signature for the data, signed with the private key
 * @returns {Boolean} true if supplied signature matches the signed data
 */
function signatureMatchesSignedData(base64PublicKey, signedData, signature) {
    if (typeof base64PublicKey !== 'string') throw new Error("ERROR: base64PublicKey (string) must be supplied");
    if (typeof signedData      !== 'string') throw new Error("ERROR: signedData (string) must be supplied");
    if (typeof signature       !== 'string') throw new Error("ERROR: signature (string) must be supplied");
    return FF_JS_API.signatureMatchesSignedData(base64PublicKey, signedData, signature);
}
exports.signatureMatchesSignedData = signatureMatchesSignedData;

/**
 * publish messages which are delivered in real-time via web-sockets to subscribing clients.
 * <br><strong>NB: <strong>Currently in alpha testing - please contact us if you wish to join the alpha trial
 * @param {String} topicName - a topic name - alphanumeric characters only
 * @param {String} payload - generic payload - most usefully, a JSON string, but can be any string
 */
function realTimePublish (topicName, payload) {
    if (typeof topicName !== 'string') throw new Error("ERROR: topicName parameter must be supplied (string)");
    if (typeof payload   !== 'string') throw new Error("ERROR: payload parameter must be supplied (string)");
    FF_JS_API.realTimePublish(topicName, payload);
}
exports.realTimePublish = realTimePublish;

/**
 * Enables you to safely execute asynchronous code, server-side
 * @param {Function} callback the function which will be executed after the timeout has expired
 * @param {Number} timeoutInMillis
 * @param param can be anything (object, array, function, ... )
 */
exports.setTimeout = function (callback, timeoutInMillis, param) {
    _setTimeout (wrapper, timeoutInMillis, param);
    function wrapper(_param) {
        try {
            //noinspection JSUnresolvedVariable,JSUnresolvedFunction
            FF_JS_API.beginInteraction();
            callback(_param);
        } finally {
            //noinspection JSUnresolvedVariable,JSUnresolvedFunction
            FF_JS_API.endInteraction();
        }
    }
};
